...
namespace Ethyme
{
	...
	*/
	...
	public:
		...
		*/
		Command(const std::function<cppcoro::task<>(Structures::Message& message, std::unordered_map<std::string, Argument> arguments)>& callback, std::unordered_map<std::string, Command::Argument> args);
		/**
		...
		*/
		 ...
		 * @return A function
		*/
		const std::function<cppcoro::task<>(Structures::Message& message, std::unordered_map<std::string, Argument> arguments)>& Callback() const;

	protected:
		std::unordered_map<std::string, Argument> arguments;

	private:
		std::function<cppcoro::task<>(Structures::Message& message, std::unordered_map<std::string, Argument> arguments)> m_callback;
	};
}